...
final_function = function(Y,X,Z,tau,B) {
  ## Final Function that wraps everything together
  ## Y    : response var (nx1)
  ...
  a <- KMQR.test.new(Y,X,Z,tau,1)
  b <- KMQR.test.new(Y,X,Z,tau,2)
  Tobs <- log(a) + log(b)

  Tobs2 <- replicate(B,strap(tau,Y,X,Z))
  
  return((sum(Tobs2 < Tobs) + 1)/(B+1) )
  
}

strap <- function(tau,Y,X,Z){
  ## Bootstraps the p-values
  ## tau  : quantile level
  ## Y    : response var (nx1)
  ## X    : nonparamteric var (nxp)
  ## Z    : parametric var (nxq)
  
  ystar <- Y[sample(1:length(Y))]
  astar <- KMQR.test.new(ystar,X,Z,tau,1)
  bstar <- KMQR.test.new(ystar,X,Z,tau,2)
  return(log(astar) + log(bstar))
  
}

## pow is the power you raise the kernel to (we are trying one here)

KMQR.test.new = function(Y, X, Z, tau, pow) {
  ## fast Kernel machine quantile regression
  ## Y    : response var (nx1)
  ## X    : nonparamteric var (nxp)
  ## Z    : parametric var (nxq)
  ## tau  : quantile level
  ## pow  : kernel you are raising the function to
  
  n <- length(Y)
  
  K <- (1 + X %*% t(X))^pow 
  
  # if statement checks for special case when there are no covariates
  if(length(Z) == 1) {
    quantresult <- rq(Y~1, tau=tau)
  } else {
